...
const GlobalSearch: React.FC = () => {
  ...
  const fetchSearchResults = async (): Promise<SearchResult> => {
    const response = await fetch(`https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/us-colleges-and-universities/records?where=name%20like%20%22${encodeURIComponent(searchTerm.toLowerCase())}%22&limit=20`)
    if (!response.ok) {
            ...
            {data.results.map((school) => (
              ...
              <BackgroundGradient className="rounded-[22px] max-w-sm p-4 sm:p-10 bg-white dark:bg-zinc-900">
                ...
                <button className="rounded-full pl-4 pr-4 py-1 text-white flex items-center space-x-1 bg-black mt-4 text-xs font-bold dark:bg-zinc-800">
                  <span>Pop: {parseInt(school.population) < 0 ? 'N/A' : school.population}</span>
